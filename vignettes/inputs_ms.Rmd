...
number_zones_per_country <- apply(with(dat, table(country, zone) > 0), 1, sum)
number_zones_per_region  <- apply(with(dat, table(subregion, zone) > 0), 1, sum)
number_zones_per_species <- apply(with(dat, table(region, zone) > 0), 1, sum)
...
zones_per_species <- dlply(dat, .(region, subregion, country), summarize, zones = as.character(unique(zone)))
zones_per_species <- lapply(names(number_zones_per_species), function(x) as.character(unlist(zones_per_species[grep(x, names(zones_per_species))])))
names(zones_per_species) <- names(number_zones_per_species)
...
# order data by region, subregion, country then zone
dat <- dat[order(dat$region, dat$subregion, dat$country, dat$zone),]
...
# extract species
species <- names(zones_per_species)
...
for (i in species) {
    ...
    stopifnot(all(unique(tmp$zone) == zones_per_species[[i]]))
