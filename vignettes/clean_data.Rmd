...
```{r, echo=TRUE}
# load data
load(file.path("../data-raw", "GLOBAL_AED_1979_2016_raw.rda"))
```
...
if (with(dat, any(is.na(country) | country == ""))) {
    warning("deleting unknonwn countries")
...
for (i in countries) {
    ...
    dfr.tmp <- ddply(dfr, .(zone), summarize, region = region, year = year, survey_area = survey_area, 
                     ...
                     diff_from_mode_D20 = !red::trim(x = survey_area, delta = 0.2)[['trim']], 
                     modal_survey_area = rep(red::trim(x = survey_area), length = length(survey_area)),
    ...
    # identify surveys that are 40% different and (for the revised density)
	  ...
	  # mark for deletion
    dfr.tmp <- ddply(dfr, .(zone), summarize, region = region, year = year, survey_area = survey_area,
                     diff_from_mode_D40 = !red::trim(x = survey_area, delta = 0.4)[['trim']], 
    ...
    dfr$revised_density[dfr$delete_revised] <- NA
	  ...
	  dfr$delete_revised <- NULL
	  dfr$delete         <- NULL
    
    # count number surveys per zone
    dfr.tmp <- ddply(dfr, .(zone), summarize, n_surveys = sum(!is.na(density)), n_surveys_revised = sum(!is.na(revised_density)))
...
Delete sites NA density
```{r}
dat <- subset(dat, !is.na(density))
```
...
Write .csv file
```{r}
write.csv(dat, file = file.path("../data-raw", "GLOBAL_AED_1979_2016.csv"), row.names = FALSE)
```
...
# save clean data
AED <- dat
save(AED, file = file.path("../data", "AED.rda"))
```
